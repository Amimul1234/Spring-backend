...
public interface ProductRepository extends JpaRepository<Owo_product, Integer> {
    @Query("SELECT e FROM Owo_product e WHERE e.product_category IN (:categories) order by e.product_price")
    Page<Owo_product> findByCategories(@Param("categories") List<String> categories, Pageable pageable);

    @Query("SELECT e FROM Owo_product e WHERE e.product_category IN (:categories) order by e.product_price DESC")
    Page<Owo_product> findByCategoriesDesc(@Param("categories") List<String> categories, Pageable pageable);
